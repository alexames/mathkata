 ...
 */
#ifndef MATHKATA_VECTOR_H_
#define MATHKATA_VECTOR_H_
...
/// @file mathkata/vector.h Vector
/// @brief Vector class and functions.
/// @addtogroup mathkata_vector

// Disable spurious warnings generated by MATHKATA_VECTOR_OPERATION().
#if defined(_MSC_VER)
#pragma warning(push)
...
#define MATHKATA_VECTOR_OPERATION(OP) MATHKATA_UNROLLED_LOOP(i, Dims, OP)
/// @endcond
...
/// @cond MATHKATA_INTERNAL
#define MATHKATA_VECTOR_SELF_OPERATOR(OP) \
  {                                       \
    MATHKATA_VECTOR_OPERATION(OP);        \
    return *this;                         \
  }
/// @endcond
...
/// @class VectorPacked "mathkata/vector.h"
...
struct VectorPacked {
  ...
  ///
  /// Default-initialization (VectorPacked<T, Dims> v;) leaves the elements
  /// indeterminate; value-initialization (VectorPacked<T, Dims> v{}) zeroes the
  /// elements.
...
/// @addtogroup mathkata_vector
/// @{
/// @class Vector "mathkata/vector.h"
...
class Vector {
 public:
  ...
  ///
  /// Default-initialization (Vector<T, Dims> v;) leaves the elements
  /// indeterminate; value-initialization (Vector<T, Dims> v{}) zeroes the
  /// elements.
  ...
  explicit constexpr Vector(const Vector<U, Dims>& v) {
    MATHKATA_VECTOR_OPERATION(data_[i] = static_cast<T>(v[i]));
  }
  ...
  explicit constexpr Vector(T s) { MATHKATA_VECTOR_OPERATION(data_[i] = s); }
  ...
  explicit constexpr Vector(const T* a) {
    MATHKATA_VECTOR_OPERATION(data_[i] = a[i]);
  }
  ...
  explicit constexpr Vector(const VectorPacked<T, Dims>& vector) {
    MATHKATA_VECTOR_OPERATION(data_[i] = vector.data_[i]);
  }
...
constexpr Vector<T, Dims> operator*(T s, const Vector<T, Dims>& v) {
  MATHKATA_VECTOR_OPERATOR(v.data_[i] * s);
}
...
constexpr Vector<T, Dims> operator/(const T& s, const Vector<T, Dims>& v) {
  MATHKATA_VECTOR_OPERATOR(s / v.data_[i]);
}
...
constexpr Vector<T, Dims>& operator+=(Vector<T, Dims>& lhs,
                                      const Vector<T, Dims>& rhs) {
  MATHKATA_VECTOR_OPERATION(lhs.data_[i] += rhs[i]);
  return lhs;
}
...
constexpr Vector<T, Dims>& operator*=(Vector<T, Dims>& v, T s) {
  MATHKATA_VECTOR_OPERATION(v.data_[i] *= s);
  return v;
}
...
#include "mathkata/internal/vector_2_simd.h"
#include "mathkata/internal/vector_3_simd.h"
#include "mathkata/internal/vector_4_simd.h"
